...
This example from the textbook (Section 7.1) illustrates a *pizza delivery web site* with three major steps in the ordering process:

+ *Order*: The user requests `order.html` from the web server, which contains an order form allowing the user to choose the number of pizzas and input credit card details. The form is processed by a `confirm_order` script on the server. Assume the user wants one pizza costing \$5.50.

+ *Confirmation*: The server sends back an HTML confirmation form generated by `confirm_order`. This form displays the total cost and presents "yes" / "no" buttons. Critically, the price is embedded in the HTML as a hidden field:
  ```html
  <INPUT TYPE="hidden" NAME="price" VALUE="5.50">
  ```
  When the user clicks "yes," the browser issues an HTTP request like:
  ```
  GET /submit_order?price=5.50&pay=yes HTTP/1.0
  ```

+ *Fulfillment*: The server's `submit_order` script (shown on #link(<page_109>, "Page 109")) receives this request, extracts the `pay` and `price` variables directly from the HTTP request parameters, and then attempts to authorize a credit card charge for `price` dollars. If successful, it settles the transaction and dispatches a delivery person.

The core vulnerability here is that the `HIDDEN` input field (from #link(<page_108>, "Page 108")) stores the price of the pizza on the _client side_. Although the field is not visually rendered by the browser, it is still present in the HTML source and is sent back to the server as part of the HTTP request. The server then _trusts_ this client-supplied value to charge the credit card --- meaning the server has delegated pricing authority to the client, which it should never do.

#note[Even if the connection uses SSL/TLS, this does *not* prevent client-state manipulation. SSL only protects data in transit between client and server; it does not prevent the client (or an attacker on the client side) from modifying the data before it is sent.]

So what could a malicious user do to exploit this? Several things:

1. *Modify the HTML source directly*: Using the browser's developer tools (e.g., "Inspect Element"), the user could change the hidden field's value from `5.50` to `0.01`, or even a negative value like `-5.50` (potentially crediting their account). This is trivial --- modern browsers make the DOM fully editable in real time.

2. *Craft the URL manually in the browser*: Since the form uses `METHOD="GET"`, all parameters are visible in the URL. The user could simply type `https://.../submit_order?price=0.01&pay=yes` directly into the browser's address bar, completely bypassing the HTML form and any client-side validation.

3. *Use command-line tools like cURL or WGET*: The user could craft arbitrary HTTP requests from the command line, for example:
  ```bash
  curl "https://.../submit_order?price=0.01&pay=yes"
  ```
  This differs from approach (2) because tools like cURL provide full control over every aspect of the HTTP request --- headers, cookies, request method, body content, etc. --- enabling more sophisticated manipulations beyond what the browser address bar allows.

The fundamental problem, as shown in the server pseudocode on #link(<page_109>, "Page 109"), is that `price` is retrieved directly from the HTTP request and passed straight to `authorize_credit_card_charge(price)` without any server-side validation or re-computation. The server _should_ have recomputed the price from its own records (e.g., looking up the item price in a database) rather than trusting the value sent by the client.
...
#note[
  Notes hereon taken on Mon Apr 13, 2026
]

== Session ID and Cookie Attacks, CSRF, and XSRF

#definition()[
  *CSRF (Cross-Site Request Forgery)*

  *Ligatti*: An attack in which a privileged client (the victim's web broswer --- which has acces to a session id (sid) cookie) sends a request to a vulnerable server (the bank's website) that causes the server to perform an action on behalf of a less-privileged 3rd party attacker. This is an example of a confused deputy attack, where the victim's browser is the "confused deputy" that is tricked into performing an action on behalf of the attacker.

  *Non-Ligatti*: An attack where a malicious website tricks a user's browser into making an unwanted request to a different site where the user is authenticated. For example, if a user is logged into their bank's website and then visits a malicious website, that malicious website could contain code that makes a request to the bank's website to transfer money from the user's account to the attacker's account without the user's knowledge or consent.

  This attack relies on the fact that the user's browser automatically includes cookies (such as session IDs) with requests to the bank's website, allowing the malicious request to be authenticated as if it were coming from the user.

  To mitigate CSRF attacks, web applications can implement anti-CSRF tokens, which are unique tokens generated by the server and included in forms or requests. The server then verifies that the token is present and valid before processing the request, ensuring that it originated from a legitimate source.
]

#definition()[
  *XSRF (Cross-Site Request Forgery)*

  Another term for CSRF. Some people use XSRF instead of CSRF to avoid confusion with "cross-site scripting" (XSS) attacks, which are a different type of attack that involves injecting malicious scripts into web pages viewed by other users. Both terms refer to the same type of attack where a malicious website tricks a user's browser into making an unwanted request to a different site where the user is authenticated.
]

#question()[
  *But why is it called "Cross-Site" Request Forgery?*

  The term "Cross-Site" refers to the fact that the attack involves a request being made from one site (the malicious website) to another site (the vulnerable server, such as the bank's website). The attack exploits the trust that the vulnerable server has in the user's browser, which automatically includes cookies (such as session IDs) with requests to the vulnerable server. The "Request Forgery" part of the name refers to the fact that the attack involves forging a request that appears to come from the user, but is actually initiated by the malicious website without the user's knowledge or consent.
]

But what are some defenses against CSRF/XSRF attacks? There are several approaches that web applications can take to mitigate the risk of CSRF attacks:
1. *CSRF Tokens*: Where the server embeds a hidden, (e.g. used in a "POST thing"---Ligatti) hard to guess, token in HTML forms or requests and only satisfies requests that include that valid token.
  - This is another way to authenticate the request, in addition to the session ID. The server can check that the CSRF token is valid and matches the expected value for the user's session before processing the request.
  - You definitely don't want to send this in a cookie because cookeis are just a different type of session ID, and you don't want to have multiple session IDs (one for the session and one for the CSRF token) that are both sent in cookies. Instead, the CSRF token should be included in the body of the request (e.g., as a hidden form field) or in a custom header, so that it is not automatically sent with every request like a cookie would be.
...
#definition()[
  *Database Management System (DBMS)*
  ...
  A database management system (DBMS) is software that provides an interface for users and applications to interact with a database. It allows users to create, read, update, and delete data in the database, as well as manage the structure and organization of the data. Examples of DBMS include MySQL, PostgreSQL, Oracle Database, Microsoft SQL Server, MongoDB, and SQLite.
]

#definition()[
  *Relational Database*

  A type of database that organizes data into tables with rows and columns, where each row represents a record and each column represents a field. Relational databases use Structured Query Language (SQL) for managing and querying the data. Examples of relational databases include MySQL, PostgreSQL, Oracle Database, and Microsoft SQL Server.

  In this model,
  - Data is stored into tables (relations)
  - You have a unique key for each row (primary key)
  - Managed by RDBMS (Relational Database Management System)
]

There are other ways to store data, too:
- *XML*: eXtensible Markup Language, which is a markup language that defines a set of rules for encoding documents in a format that is both human-readable and machine-readable. It is often used for storing and transporting data, and it can be used to represent complex data structures.
- *XML Database*: A documented oriented database that uses XML to store data. It allows for flexible and hierarchical data storage, making it suitable for applications that require complex data structures or need to handle semi-structured data.

The professor notes that SQL databases are relational. Additionally, there are some query languages that are for non-relational databases, such as NoSQL (MongoDB, Amazon DynamoDB, etc.) and GraphQL (used for APIs to query data in a more flexible way than traditional REST APIs).

Moreover, every relation has a *schema*. A *logical schema* defines many types and names. Similarly, a *physical schema* defines data layouts, or data structures---D.S.s

*Tables encode the entiries (objects) and the relationships (between entities).*

Consider the example of some inventory database:
...
CREATE TABLE purchases (
  ...
  sku VARCHAR(255) REFERENCES inventory(sku) ON DELETE CASCADE,
  time TIMESTAMP NOT NULL,
  ... # ...and so on
);
```

There's a widely used notation when writing out a *logical schema*, or schema.

#important()[
  The professor notes that "logical schema" always is the same as "schema" in this course.
]

How do you write out the logical schema for the above `inventory` table?
$
  "inventory"(underline("sku": "string"), "name": "string", "price": "float", "brand": "string") \
  "customers"(underline("customer_id": "int"), "name": "string") \
  "purchases"(underline("purchase_id": "int"), "customer_id": "int", "sku": "string", "time": "timestamp")
$

#definition()[
  *Row/Tuple/Record*

  An individual entry in a table, which represents a single instance of the entity being modeled. For example, in the `inventory` table, each row would represent a specific product with its corresponding SKU, name, price, and brand.
]

#definition()[
  *Set*

  An entire table, which is a collection of rows (or tuples/records) that share the same structure defined by the schema. For example, the `inventory` table as a whole can be considered a set of all products in the inventory, where each product is represented by a row in the table.
]

#definition()[
  *Column/Attribute*

  A specific field or property of the entity being modeled, which corresponds to a particular piece of data in each row. For example, in the `inventory` table, "sku", "name", "price", and "brand" are all columns (or attributes) that describe different aspects of each product.
]

Note that each column has to be a primitive type (e.g., string, int, float, timestamp, etc.) and cannot be a complex data structure. This is a key characteristic of relational databases, which require that each attribute in a table be atomic (i.e., indivisible) to maintain the integrity of the data and ensure that it can be easily queried and manipulated using SQL.

== SQL Code

SQL is a DDL and a DML language.

#definition()[
  *DDL (Data Definition Language)*

  A subset of SQL that is used to define and manage the structure of a database, including creating, altering, and dropping tables, as well as defining relationships between tables. Examples of DDL statements include `CREATE TABLE`, `ALTER TABLE`, and `DROP TABLE`.

  In the example above, the `CREATE TABLE` statements are examples of DDL commands that define the structure of the `inventory`, `customers`, and `purchases` tables.
]

#definition()[
  *DML (Data Manipulation Language)*

  A subset of SQL that is used to query and manipulate data within a database. DML statements include `SELECT`, `INSERT`, `UPDATE`, and `DELETE`, which allow users to retrieve, add, modify, and remove data from the database, respectively.

  For example, a DML statement to retrieve all products from the `inventory` table would be:
  ```sql
...
Several operatoions, the main ones are covered here:
...
#note[
  SQL is case insensitive. So, for example, `CREATE TABLE` is the same as `create table` or `CrEaTe TaBlE`. However, it is common practice to write SQL keywords in uppercase for readability and to distinguish them from other identifiers (such as table names and column names), which are often written in lowercase.
]
...
3. Alter (changing) a table:
```sql
ALTER TABLE inventory ADD COLUMN description VARCHAR(255);
```
This will add a new column called `description` to the `inventory` table.

```sql
ALTER TABLE inventory DROP COLUMN description;
```
This will drop the `description` column from the `inventory` table.

```sql
ALTER TABLE inventory ALTER COLUMN price DECIMAL(12, 2);
```
This will change the data type of the `price` column to `DECIMAL(12, 2)`.

```sql
ALTER TABLE inventory RENAME COLUMN oldName TO newName;
```
This will rename the column from `oldName` to `newName`.

=== DML (Data Manipulation Language)

1. Deleting from a table
```sql
DELETE FROM inventory;
```
This will delete all the rows *but the table still exists*; the schema is still there but not it has no rows in it.

2. Inserting into a table
```sql
INSERT INTO inventory (sku, name, price, brand) VALUES ('12345', 'Pizza', 5.50, 'PizzaBrand');
```
This will insert a new row into the `inventory` table with the specified values for each column

But what if we didn't want to put everything? We can do that, too:
```sql
